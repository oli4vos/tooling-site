...
|---|---|---|---|---|---|
| Toolbibliotheek + categorie-navigatie | actief | n.v.t. | aan | `/apps` toont alle 6 publieke tools direct in rustige taakgerichte groepen met één titel, korte taakzin en actie per kaart, zonder doelgroepfilter of links naar hidden of uitgeschakelde tools | n.v.t. |
...
| Publieke calculators (app-registry) | actief | `enabled: true` + `visibility: "public"` | aan | 6 publieke tools in de huidige registry; disabled tools tellen niet mee | n.v.t. |
...
| Submit-driven calculatorflow | actief | `useMobileFieldFlow` + `MobileFieldFlowControls` + `CalculationResultActions` | aan | Alle 6 publieke calculators tonen mobiel één relevante vraag per stap met conditionele voortgang, gerichte Enter-afhandeling, focusherstel en een sticky safe-area-actiebalk. Optionele verdiepingen tellen niet als lege vraag. Resultaten bieden centraal invoer wijzigen en bevestigd opnieuw beginnen; calculatoren delen pas vanaf 1280 px in twee kolommen | n.v.t. |
...
| DUO-impact in maximale-hypotheektool | voorbereid | uit via manifest van Maximale hypotheek | uit in publieke UI | De bewaarde tool toont bij heractivatie dezelfde centrale DUO-impact en berekeningsopbouw; formules en tests blijven behouden | heractiveert samen met Maximale hypotheek |
| Hypotheekrentelink en salarisverhogingsanalyse | voorbereid | centrale linkconfig + salarisadapter, uit via toolmanifest | uit in publieke UI | De bewaarde maximale-hypotheekflow bevat de rentelink en salarisverhogingsanalyse; de centrale hypotheekengine blijft ongewijzigd | heractiveert samen met Maximale hypotheek |
