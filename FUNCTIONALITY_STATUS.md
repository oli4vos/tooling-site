...
| Toolbibliotheek + categorie-navigatie | actief | n.v.t. | aan | `/apps` toont 18 publieke tools, gegroepeerd rond inkomen, vermogen, ondernemen, wonen, vervoer, studie en lenen; hidden en uitgeschakelde tools blijven uitgesloten | n.v.t. |
...
| Publieke calculators (app-registry) | actief | `enabled: true` + `visibility: "public"` | aan | 18 publieke tools in de huidige registry; disabled tools tellen niet mee | n.v.t. |
...
| Submit-driven calculatorflow | actief | `useMobileFieldFlow` + `MobileFieldFlowControls` + `CalculationResultActions` | aan | Alle 18 publieke calculators tonen mobiel één relevante vraag per stap met conditionele voortgang, gerichte Enter-afhandeling en resultaatfocus. Optionele verdiepingen tellen niet als lege vraag | n.v.t. |
